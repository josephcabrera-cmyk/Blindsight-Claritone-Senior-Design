...
int main(void)
{
  ...
  uint32_t pitch_nn = 0;
  CameraPipeline_Init((uint32_t *[2]) {&lcd_bg_area.XSize, &lcd_fg_area.XSize},
                      (uint32_t *[2]) {&lcd_bg_area.YSize, &lcd_fg_area.YSize},
                      &pitch_nn);
  ...
  printf("Claritone UART detection logging enabled\n");
  printf("========================================\n");
  ...
  while (1)
  {
    ...
    /* Optional: keep this call if you later attach a display.
       If you want a cleaner headless build, comment this out. */
...
static void Claritone_LogDetections(const od_pp_out_t *p_postprocess)
{
  ...
  for (uint32_t i = 0; i < nb_rois; i++)
  {
    ...
    printf("DET %lu: class=%ld label=%s conf=%.0f%% x=%.2f y=%.2f w=%.2f h=%.2f zone=%s\r\n",
           ...
           classes_table[rois[i].class_index],
           rois[i].conf * 100.0f,
           rois[i].x_center,
           rois[i].y_center,
           rois[i].width,
...
static void Claritone_HandleFrame(const od_pp_out_t *p_postprocess)
{
  ...
  /* Print every 10th frame to reduce UART spam */
  if ((frame_counter % 10U) == 0U)
  {
    Claritone_LogDetections(p_postprocess);
  }

  claritone_target_t best_person = Claritone_SelectBestPerson(p_postprocess);
  ...
  if (best_person.valid)
  {
    ...
    /* Future Claritone hook:
       - trigger ToF read here
       - convert ToF distance + zone into sound cue
       Example:
         uint16_t dist_mm = TOF_ReadMM();
         Claritone_PlayCue(best_person.zone, dist_mm);
    */
  }
}
...
static void Display_NetworkOutput(od_pp_out_t *p_postprocess, uint32_t inference_ms)
{
  ...
  UTIL_LCD_FillRect(0, 0, lcd_fg_area.XSize, lcd_fg_area.YSize, UTIL_LCD_COLOR_TRANSPARENT);

  for (int32_t i = 0; i < (int32_t)nb_rois; i++)
  {
...
static void Display_init(void)
{
  ...
  for (int i = 0; i < (int)(sizeof(screen_buffer) / 4); i++)
  {
